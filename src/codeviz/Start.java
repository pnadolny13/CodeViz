				...
				if (child.toString().endsWith(".java")) {
					...
					try {
						...
						while (it.hasNext()) {
							...
							if (line.contains("public class") || line.contains("protected class")
								...
								wr.newLine();
								wr.write("import java.io.IOException;");
								wr.newLine();
								wr.newLine();
							...
							// if the line is a method, set method name ... close bracket means the class is closing
							else if (bracketCount == 1 && !line.contains("}")) {
								// this is a method line...extract
								String[] methodLineContents = line.split("\\(");
								String[] method = methodLineContents[0].split(" ");
								String actualmethod = method[method.length - 1];
								methodName = StringUtils.remove(actualmethod, "()");

								// if its the same as class name, its a contructor so dont add codeViz line
								if (!methodName.equals(className)) {
									String codeVizLine = "\"" + methodName + " from " + className + " was executed" + "\"";
									// add throws IOException before the close bracket
									wr.write(line.replace("{", " throws IOException {"));
									wr.newLine();
							...
							if (line.contains("{")) {
								int ups = StringUtils.countMatches(line, "{");
								bracketCount += ups;
							}
							if (line.contains("}")) {
								int downs = StringUtils.countMatches(line, "}");
								bracketCount -= downs;
							}
						}
